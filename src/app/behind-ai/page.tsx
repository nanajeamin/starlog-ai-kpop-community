...
const EXAMPLE_OUTPUT = `{
  ...
  "cloud_generation_prompt": "A young person standing near floor-to-ceiling windows...",
  "priority_scores": {
...
const STEPS = [
  { num: "01", title: "原始文案输入", desc: "接收粉丝攻略、官方推文、媒体报道等非结构化文本", bg: "bg-block-lilac" },
  { num: "02", title: "LLM 语义解析", desc: "claude-sonnet-4-6 通过 Tool Use 强制输出，解析地点、爱豆、拍照建议等信息", bg: "bg-block-teal" },
  { num: "03", title: "结构化模板生成", desc: "输出标准 IdolSpotTemplate 格式，包含 3 维标签和优先级评分", bg: "bg-block-yellow" },
  { num: "04", title: "人工审核上线", desc: "运营团队验证信息真实性后，模板进入地图和生成系统", bg: "bg-block-coral" },
];
    ...
    <div className="min-h-screen bg-canvas">
      <div className="bg-block-navy">
        <div className="section-container py-20">
          ...
          <p className="text-base text-white/60 max-w-2xl">K-Star Spot 的内容由 AI Agent 自动提取生成。我们将粉丝攻略、官方社媒、媒体报道转化为结构化的「同款模板」，驱动地图、路线规划和云同款生成三大功能。</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-block-coral text-[#600000]">输入 · 原始粉丝攻略</span>
              </div>
              <pre className="bg-canvas rounded-2xl border border-hairline p-6 text-sm text-slate font-mono leading-relaxed overflow-x-auto whitespace-pre-wrap">{EXAMPLE_INPUT}</pre>
            </div>
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-block-teal text-[#007A92]">输出 · 结构化模板 JSON</span>
                <span className="text-xs text-stone">claude-sonnet-4-6 · Tool Use</span>
              </div>
              <pre className="bg-[#1a1a3e] rounded-2xl p-6 text-sm text-[#c3faf5] font-mono leading-relaxed overflow-x-auto whitespace-pre-wrap text-xs">{EXAMPLE_OUTPUT}</pre>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="section-container py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
          {[
            { title: "LLM：claude-sonnet-4-6", desc: "使用 Tool Use + tool_choice 强制 JSON 输出，确保结构化数据稳定，不产生幻觉", bg: "bg-block-lilac" },
            { title: "三维标签体系", desc: "构图标签（构图角度/位置）× 色调标签（光线/风格）× 姿势标签（站/坐/持物）", bg: "bg-block-teal" },
            { title: "优先级评分", desc: "6 维打分：爱豆相关度、热度、稀缺性、出片质量、新鲜度、可信度，加权综合排序", bg: "bg-block-yellow" },
          ].map((c) => (
      ...
      <div className="section-container pb-16">
        <div className="bg-block-yellow rounded-3xl p-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {["❌ 不生成爱豆本人形象","❌ 不生成用户与爱豆的合照","❌ 不伪造爱豆出现在新场景","❌ 不存储 / 复用 / 二次分发爱豆原图","✅ 仅提取场景/构图/色调等抽象描述","✅ 生成对象始终是用户本人","✅ 所有结果标注「AI Generated · K-Star Spot」","✅ 屏蔽明星脸 / 未成年人 / NSFW 内容"].map((item) => (
      ...
      <div className="section-container pb-20">
        <div className="text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/map" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-ink text-white text-sm font-medium hover:bg-charcoal transition-colors">🗺️ 探索圣地地图</Link>
            <Link href="/generate" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full border border-hairline-strong text-ink text-sm font-medium hover:bg-surface transition-colors">✨ 生成同款照</Link>
          </div>
        </div>
      </div>
    </div>
