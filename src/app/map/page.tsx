        ...
        <div className="section-container py-3 space-y-2">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            <button
              ...
              className={`pill-tab flex-shrink-0 ${activeGroup === "all" ? "active" : ""}`}
            >
              全部团体
            </button>
            {groups.map((g) => (
              <button
                ...
                className={`pill-tab flex-shrink-0 ${activeGroup === g.id ? "active" : ""}`}
              >
                {g.name}
              </button>
            ))}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            <button
              ...
              className={`pill-tab flex-shrink-0 text-xs py-1.5 px-3 ${activeType === "all" ? "active" : ""}`}
            >
              全部类型
            </button>
            {PLACE_TYPES.map(([type, label]) => (
              <button
                ...
                className={`pill-tab flex-shrink-0 text-xs py-1.5 px-3 ${activeType === type ? "active" : ""}`}
              >
                {label}
              </button>
            ))}
      ...
      <div className="section-container py-6">
        <div className="flex flex-col lg:flex-row gap-6" style={{ minHeight: "calc(100vh - 280px)" }}>
                  ...
                  <div
                    ...
                    className={`rounded-2xl border overflow-hidden cursor-pointer transition-all ${
                      isSelected
                        ...
                        : "border-hairline-soft hover:border-hairline-strong hover:shadow-subtle"
                    }`}
                  >
                    <div className={`h-24 bg-gradient-to-br ${gradient} relative flex items-end p-3`}>
                      ...
                      <div
                        className="absolute top-3 right-3 w-10 h-10 rounded-full opacity-25"
                        style={{ backgroundColor: config?.color }}
                      />
                    </div>
                    <div className="bg-canvas p-4">
                      <p className="text-xs font-mono uppercase tracking-wider text-stone mb-1">
                        {PLACE_TYPE_LABELS[t.place_type]}
                      </p>
          ...
          <div className="flex-1 min-h-[400px] lg:min-h-0">
            <MapPlaceholder templates={filtered} onSelect={setSelected} />
          </div>
        </div>
      </div>
    </div>
